	...
	return (
		<>
			<nav className="absolute z-30 flex flex-row items-center justify-center p-4 font-black text-white text-l sm:text-2xl top-4 left-4">
				...
				<Link href="/?skipanimation" className="mx-3 link-underline sm:hidden">Home</Link>
				<Link href="/about" className={`${router.asPath.startsWith('/about') ? 'text-red-500 cursor-default' : 'link-underline'} mx-3`}>About</Link>
				<Link href="/projects" className={`${router.asPath.startsWith('/projects') ? 'text-yellow-500 cursor-default' : 'link-underline'} mx-3`}>Projects</Link>
				<Link href="/contact" className={`${router.asPath.startsWith('/contact') ? 'text-blue-500 cursor-default' : 'link-underline'} mx-3`}>Contact</Link>
			</nav>
